...
VAULT_LEADS = Path.home() / "Library/Mobile Documents/iCloud~md~obsidian/Documents/CLAUDE/CLAUDE/1 - Projects/The Pit Room/Leads/leads-log.md"

VAULT_HEADER = "# Leads Log\n\nCaptured from the demo site's lead form. Newest first.\n\n"
...
def prepend_lead_to_vault(entry: dict):
    if not VAULT_LEADS.parent.exists():
        return  # vault not present on this machine, skip silently
    block = (
        f"## {entry['received_at']} — {entry.get('name', '(no name)')}\n"
        f"- Phone: {entry.get('phone', '')}\n"
        f"- Neighborhood: {entry.get('neighborhood', '')}\n"
...
if __name__ == "__main__":
    ...
    print(f"Serving demo site on http://127.0.0.1:{port}")
    httpd.serve_forever()
